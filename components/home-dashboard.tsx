...
function TabLoadingSkeleton({ name }: { name: string }) {
  return (
    <div className="w-full min-h-[400px] flex flex-col items-center justify-center p-8 bg-card rounded-3xl border border-border/40 shadow-xs animate-pulse">
...
export default function HomeDashboard() {
  ...
  if (!mounted) {
    return <TabLoadingSkeleton name="Loading" />
  }
  ...
  return (
    <div className="w-full">
      <Tabs 
        ...
        className="w-full max-w-4xl mx-auto pb-24 sm:pb-0"
      >
        ...
        <AnimatePresence mode="wait">
          <TabsContent value="alarm" className="mt-0 outline-none" key="alarm-content">
            <motion.div
          ...
          <TabsContent value="timer" className="mt-0 outline-none" key="timer-content">
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
          ...
          <TabsContent value="stopwatch" className="mt-0 outline-none" key="stopwatch-content">
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
          ...
          <TabsContent value="clock" className="mt-0 outline-none" key="clock-content">
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
      ...
      <div className="sm:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center bg-background/95 backdrop-blur-md border rounded-full shadow-2xl p-1.5 gap-1.5">
        ...
        <Button variant={activeTab === 'alarm' ? 'secondary' : 'ghost'} size="icon" className="rounded-full" onClick={() => {
          setActiveTab('alarm');
        }}>
            <AlarmClock className="w-5 h-5"/>
        </Button>
        <Button variant={activeTab === 'timer' ? 'secondary' : 'ghost'} size="icon" className="rounded-full" onClick={() => {
          setActiveTab('timer');
        }}>
            <Hourglass className="w-5 h-5"/>
        </Button>
        <Button variant={activeTab === 'stopwatch' ? 'secondary' : 'ghost'} size="icon" className="rounded-full" onClick={() => {
